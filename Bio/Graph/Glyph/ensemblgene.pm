package Bio::Graphics::Glyph::ensemblgene;
...
sub bump {
  ...
  my $bump;
  if ($self->{level} == 0
      && lc $self->feature->primary_tag =~ /gene/
      && eval {($self->subfeat($self->feature))[0]->type =~ /transcript|RNA|pseudogene/i}) {
...
sub draw_connectors {
  my $self = shift;
  if ($self->feature->primary_tag =~ /gene/) {
      my @parts = $self->parts;
...
sub _subfeat {
  ...
  if ($feature->primary_tag =~ /gene/i) {
    my @transcripts;
    for my $t (qw/mRNA transcript tRNA snRNA snoRNA miRNA ncRNA pseudogene/) {
  ...
  elsif ($feature->primary_tag =~ /mRNA|transcript/i) {
